...
func (m *postgresDBRepo) Authenticate(email, testPassword string) (int, string, error) {
	...
	row := m.DB.QueryRowContext(ctx, "select id, passsword from users where email = $1", email)
...
func (m *postgresDBRepo) AllReservations() ([]models.Reservation, error) {
	...
	query := `
		select r.id, r.first_name, r.last_name, r.email, r.phone, r.start_date, r.end_date, r.room_id, r.processed, r.created_at, r.updated_at, rm.id, rm.room_name
		from reservations r
